...
def create_dataloader(args, datasets, tokenizer, train=False):
    ...
    # datasets[key] = dataset(data['example'][key], data['label'][key])
    # key=='attribute0'/'attribute1'/'neutral'
    ...
    # max_size: max num of sentence in all attribute sentence list
...
# splited_data: data = {'train': {'example': {'attribute0':[]}, 'label': {'attribute0':[]}}, 'dev': {'example': {'attribute0':[]}, 'label': {'attribute0':[]}}}
...
def train(args, data, datasets, model: PreTrainedModel, original_model, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def attribute_vector_example():
        ...
        for key in distribution:
            if key != 'neutral':
                ...
                inputs, labels, inputs_attention_mask, labels_attention_mask = next(dataloaders[key])
    ...
    def forward(attributes_hiddens, dataloaders, key):
        ...
        if args.model_type == 'roberta':
            ...
            final_layer_hiddens = model(inputs, inputs_attention_mask).hidden_states[-1]
            if 'neutral' != key:
                with torch.no_grad():
        ...
        if args.debias_layer == 'all':
            ...
            # target_original_hiddens = all_original_hiddens
        else:
...
def main():
    ...
    model_args.prompt = False
    model_args.pre_seq_len = 40
    model_args.prefix_projection = False
    model_args.prefix_hidden_size = 512
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler(f'../log/{time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime(time.time()))}.log')],
    )
    logger.warning(
    ...
    if args.model_name_or_path:
        model = get_model(model_args, TaskType.SEQUENCE_CLASSIFICATION, model_config)
